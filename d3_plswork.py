...
Original file is located at
    https://colab.research.google.com/drive/1fcVCH5mWKyY6DJLsAynuB5esA28N76tO
"""

print("jai shree ram")

# Colab-specific imports (commented out for local execution)
# from google.colab import drive
# drive.mount('/content/drive')

# Install dependencies (commented out - install manually if needed)
# !pip install optuna torch torchvision scikit-learn pandas numpy matplotlib seaborn scipy torch-lr-finder

import os
# os.chdir('/content/drive/MyDrive/CAPTURE')  # Colab-specific, commented out
...
def check_h3_possible(dataset_name: str):
    """
    Check if H3 validation is possible before training to avoid wasting time.
    Returns True if H3 can be run, False otherwise.
    """
    ...
    try:
        ...
        # Check if PSYT_Therapy_Use column exists
        has_therapy_col = "PSYT_Therapy_Use" in df.columns
        # Check if it's a Swiss dataset (D1_Swiss_processed or D1-Swiss)
        is_swiss = "swiss" in dataset_name.lower() or "D1" in dataset_name
        
        if has_therapy_col and is_swiss:
            print(f" H3 validation will be available for {dataset_name}")
            return True
        else:
            if not has_therapy_col:
                print(f" H3 validation NOT available: PSYT_Therapy_Use column missing in {dataset_name}")
            if not is_swiss:
                print(f" H3 validation NOT available: {dataset_name} is not a Swiss dataset")
            return False
    except Exception as e:
        print(f" Could not check H3 availability: {e}")
        return False
...
def run_autoencoder_pipeline(dataset_name: str, force_latent_dim: int = None, force_k: int = None, narrowed_params: dict = None, n_folds: int = 5):
    # Early check: Verify H3 is possible before training
    print("\n" + "="*70)
    print("PRE-TRAINING CHECK: H3 Validation Availability")
    print("="*70)
    h3_possible = check_h3_possible(dataset_name)
    print("="*70 + "\n")
    ...
    # Create progress bar for Stage 1
    pbar_stage1 = tqdm(total=total_experiments, desc="Stage 1: Architecture Tuning", unit="exp")
                ...
                for opt_name, opt_class in optimizers.items():
                    for num_epochs in epochs_list:
                        for fold_idx, (train_idx, val_idx) in enumerate(kfold.split(train_val_tensor)):
                            ...
                            config_desc = f"h={hidden_size}, l={latent_dim}, {act_name}, {opt_name}, e={num_epochs}, f={fold_idx+1}"
                            pbar_stage1.set_description(f"Stage 1: {config_desc}")
    ...
    # Create progress bar for Stage 2
    pbar_stage2 = tqdm(total=total_experiments, desc="Stage 2: Learning Params", unit="exp")
        ...
        for weight_decay in weight_decays:
            for momentum in momentum_values:
                for fold_idx, (train_idx, val_idx) in enumerate(kfold.split(train_val_tensor)):
                    ...
                    # Update progress bar description with current config
                    if best_optimizer_name == 'SGD':
                        config_desc = f"bs={batch_size}, wd={weight_decay:.0e}, mom={momentum}, f={fold_idx+1}"
                    else:
                        config_desc = f"bs={batch_size}, wd={weight_decay:.0e}, f={fold_idx+1}"
                    pbar_stage2.set_description(f"Stage 2: {config_desc}")
    ...
    # Final Model Training and Profile Extraction
    ...
    pbar_final = tqdm(range(best_epochs), desc="Final Training", unit="epoch")
    for epoch in pbar_final:
        epoch_loss = 0.0
        ...
        pbar_final.set_postfix({'loss': f'{avg_loss:.4f}'})

    print(f"\nFinal model training complete. Saving model...")
    final_model.eval()
    
    # Prepare save directory and paths
    import os
    save_dir = f"{dataset_name.replace('-', '_').replace(' ', '_')}_results"
    os.makedirs(save_dir, exist_ok=True)
    
    # Save the trained model
    model_save_path = os.path.join(save_dir, f"{dataset_name.replace('-', '_').replace(' ', '_')}_model.pth")
    torch.save({
        'model_state_dict': final_model.state_dict(),
        'best_hidden_size': best_hidden_size,
        'best_latent_dim': best_latent_dim,
        'best_activation_name': best_activation_name,
    ...
    # Check if H3 validation is possible (flexible dataset name check)
    can_run_h3 = "PSYT_Therapy_Use" in all_data_df.columns and ("swiss" in dataset_name.lower() or "D1" in dataset_name)
    ...
    if can_run_h3:
        ...
        print("\nSkipping H3 validation (PSYT_Therapy_Use column not found or not a Swiss dataset)")

    # Save Results for H3 Validation
    print("\n" + "="*70)
    print("SAVING RESULTS FOR H3 VALIDATION")
    print("="*70)
    
    import pickle
    # save_dir already created above
    
    # Save cluster assignments and test cluster assignments
    h3_data = {
        'cluster_labels_all': cluster_labels_all,  # Train+val cluster assignments
        'test_cluster_assignments': test_cluster_assignments,  # Test cluster assignments
        'cluster_centroids': cluster_centroids,  # Cluster centroids
        'best_k': best_k,
        'best_latent_dim': best_latent_dim,
        'RANDOM_SEED': RANDOM_SEED,
    ...
    print(f"✓ H3 validation data saved to: {h3_pickle_path}")
    
    # Also save as numpy .npz (more space-efficient)
    h3_npz_path = os.path.join(save_dir, 'h3_data.npz')
    np.savez(
        h3_npz_path,
    ...
    print(f"✓ H3 validation data also saved to: {h3_npz_path}")
    
    # Save full results dictionary
    results_path = os.path.join(save_dir, 'pipeline_results.pkl')
    with open(results_path, 'wb') as f:
        pickle.dump({
            ...
            'reconstruction_loss': test_recon_loss,
            'profile_summary': profile_summary,
        }, f)
    print(f"✓ Full pipeline results saved to: {results_path}")
    
    # Print summary
    print(f"\nSaved data summary:")
    print(f"  - cluster_labels_all: {cluster_labels_all.shape} (train+val)")
    print(f"  - test_cluster_assignments: {test_cluster_assignments.shape} (test)")
    print(f"  - cluster_centroids: {cluster_centroids.shape}")
    from collections import Counter
    print(f"  - Cluster distribution (train+val): {Counter(cluster_labels_all)}")
    print(f"  - Cluster distribution (test): {Counter(test_cluster_assignments)}")
    print(f"  - Model saved to: {model_save_path}")
    ...
    result = {
        ...
        'latent_vectors_all': latent_vectors_all,
        'cluster_labels_all': cluster_labels_all,
        'test_cluster_assignments': test_cluster_assignments,  # ADD THIS
        'cluster_centroids': cluster_centroids,
        'best_k': best_k,
        'best_latent_dim': best_latent_dim,
        'final_sil_score': final_sil_score,
...
def train_and_validate_model(model, train_loader, val_loader, optimizer, criterion, num_epochs, device):
    ...
    # Extract latent vectors and evaluate clustering
    model.eval()
    all_latent_vectors = []
    ...
    # K Selection via Multiple Validation Methods
...
# !pip install optuna  # Install manually if needed

import optuna
from optuna.pruners import MedianPruner

# Extract narrowed ranges from Optuna scout phase
...
# scout_results_d3_tuple = run_scout_phase(dataset_name="D3_Academic_Processed")
# scout_results_d3_dict = scout_results_d3_tuple[1]
# narrowed_params_d3 = extract_narrowed_ranges(scout_results_d3_dict)
# with open('scout_results_d3_Academic.json', 'w') as f:
#     json.dump(scout_results_d3_dict, f, indent=2)

# # Save narrowed params
# with open('narrowed_params_d3.json', 'w') as f:
#     json.dump(narrowed_params_d3, f, indent=2)

# Execution: Run Pipeline for D1_Swiss_processed

# Define narrowed parameters for D1_Swiss_processed
# If you have scout phase results, use those. Otherwise, these are reasonable defaults:
narrowed_params_d1 = {
    'hidden_sizes': [4, 5, 6, 7, 8],  # Common range for 4-feature datasets
    'latent_dims': [2, 3, 4],  # Standard latent dimensions
    'activations': ['ReLU', 'Tanh', 'Sigmoid'],  # All activations
    'optimizers': ['Adam', 'SGD'],  # Both optimizers
    'epochs': [50, 75, 100]  # Standard epochs
}

print("="*80)
print("RUNNING PIPELINE FOR D1_Swiss_processed (H3 Validation)")
print("="*80)
print(f"Narrowed parameters: {narrowed_params_d1}")
print("="*80)

# Run the pipeline
result_d1 = run_autoencoder_pipeline(
    dataset_name="D1_Swiss_processed",  # Use correct dataset name!
    narrowed_params=narrowed_params_d1,
    n_folds=10  # 10-fold CV (same as previous D1 runs)
)

print("\n" + "="*80)
print("PIPELINE COMPLETE!")
print("="*80)
print(f"Results saved to: D1_Swiss_processed_results/")
print(f"  - Model: D1_Swiss_processed_model.pth")
print(f"  - H3 data: h3_data.pkl and h3_data.npz")
print(f"  - Full results: pipeline_results.pkl")
print("="*80)
