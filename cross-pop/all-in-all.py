...
def test_cluster_similarity(pop_labels, ref_labels, k, pop_name, ref_name):
    ...
    if len(pop_trim) < 2:
        return {'reference': ref_name, 'target': pop_name, 'chi2': np.nan, 'p_value': np.nan, 'cramers_v': np.nan}
    ...
    return {'reference': ref_name, 'target': pop_name, 'chi2': chi2, 'p_value': p_value, 'cramers_v': cramers_v}
    ...
    for target_name in dataset_names:
        ...
        for cluster_id in range(ref_k):
            ...
            profile_comparisons.append({
                ...
                'Ref_Empty': False  # Since we skip empties
            })
        ...
        # RMSD between corresponding cluster profiles
        rmsd = compute_feature_deviation(ref_profiles, target_profiles, ref_k)
        
        # Semantic interpretation based on both Cramér's V and Euclidean distance
...
for name in rmsd_pivot.index:
    if name in rmsd_pivot.columns:
        rmsd_pivot.loc[name, name] = ""
print(rmsd_pivot.to_string())

# Save results
results_df.to_csv("cross_population_results_ae.csv", index=False)
...
if profile_comparisons:
    ...
    profile_comparison_df.to_csv("profile_comparison_table_ae.csv", index=False)
